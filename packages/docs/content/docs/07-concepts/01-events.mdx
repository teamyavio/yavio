...
| `status` | `success` or `error`. A call is an error when the handler throws **or** returns a result with `isError: true` (SDK 0.4.0 and later; earlier SDKs only count throws) |
| `error_category` | `tool_error` — the handler returned `isError: true`; `unknown` — the handler threw. The client sees the same shape either way, so the distinction is server-side only. Reserved: `auth`, `validation`, `timeout`, `rate_limit`, `server` |
