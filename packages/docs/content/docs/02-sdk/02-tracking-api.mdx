...
instrumented.tool("search", { query: z.string() }, async ({ query }) => {
  yavio.identify("user_123");
  yavio.track("search_executed", { query });
...
<Callout type="warn">
  The `yavio` singleton relies on `AsyncLocalStorage`. It only works within the async context of a tool handler invocation on the wrapped server. A call outside that context has no session or trace to attach to: the event is **dropped**, and the SDK warns once per process with `YAVIO-1105`. The usual cause is a tool registered on the original, unwrapped server — for example to keep its input out of analytics. Register it on the wrapped server instead and use a [per-tool override](/docs/02-sdk/03-configuration#per-tool-overrides) to limit what is captured.
</Callout>
...
```ts
instrumented.tool("wizard", { step: z.string() }, async ({ step }) => {
